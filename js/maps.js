...
$(document).ready(function(){
    ...
    objlayerBase = L.tileLayer(layerbase,paramlayerbase).addTo(maps)
        ...
        objsql.execute("SELECT name, layer FROM select_layer_copy")
            .on("done",function(data){
                    for(idx in data.rows){
                        var nombretextcapa = data.rows[idx].name;
                        var nombreorgin =  data.rows[idx].layer;
